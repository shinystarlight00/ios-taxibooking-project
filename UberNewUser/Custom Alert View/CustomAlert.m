...
- (id)initWithTitle:(NSString *)title message:(NSString *)message delegate:(id)AlertDelegate cancelButtonTitle:(NSString *)cancelButtonTitle otherButtonTitle:(NSString *)otherButtonTitle
{
    ...
    if (self) {
        ...
        UIImage *AlertBg = [UIImage imageNamed:@"alert_bg"];
        
        if ([AlertBg respondsToSelector:@selector(resizableImageWithCapInsets:)])
            AlertBg = [[UIImage imageNamed: @"alert_bg"] resizableImageWithCapInsets:UIEdgeInsetsMake(40.0, 25.0, 25.0, 25.0)];
        else
            AlertBg = [[UIImage imageNamed: @"alert_bg"] stretchableImageWithLeftCapWidth: 25 topCapHeight: 40];
        
        
        UIImage *CancelBtnImg = [UIImage imageNamed:@"close_logout"];
        UIImage *OtherBtnImg = [UIImage imageNamed:@"check_logout"];
        
        UIImageView *AlertImgView = [[UIImageView alloc] initWithImage:AlertBg];
        
        float alert_width = AlertImgView.frame.size.width;
        float alert_height = ((self.frame.size.height-AlertBg.size.height)/4.0);
        //old value 5.0
        //add text
        UILabel *TitleLbl;
        ...
        if (title)
        {
            TitleLbl = [[UILabel alloc] initWithFrame:CGRectMake(10.0, alert_height-30, alert_width-20.0, 45.0)];
            TitleLbl.adjustsFontSizeToFitWidth = YES;
            TitleLbl.font = [UberStyleGuide fontSemiBold:36.10f];
            ...
            TitleLbl.text = title;
            //5.0
            alert_height += TitleLbl.frame.size.height - 10.f;
        }
        else
            alert_height += 15.0;
        //15.0
        if (message)
        {
            ...
            MessageLbl.font = [UberStyleGuide fontRegular:19.30f];
            ...
            MsgScrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(10.0, alert_height-15, alert_width-20.0, (MessageLbl.frame.size.height>max_msg_height)?max_msg_height:MessageLbl.frame.size.height)];
        ...
        AlertView = [[UIView alloc] initWithFrame:CGRectMake((int)((self.frame.size.width-alert_width)/2.0), (int)((self.frame.size.height-[UIImage imageNamed:@"alert_bg"].size.height)/2.0), alert_width,[UIImage imageNamed:@"alert_bg"].size.height)];
